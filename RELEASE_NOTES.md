...
## Hermes Forge v0.2.33

发布日期：2026-09-17

### 核心修复与原生 Agent 对齐

- 桌面聊天继承 Hermes Profile 的人格、推理偏好、工具集、运行预算、备用模型、检查点、预置消息和路由设置，移除固定 90 回合覆盖。
- 在创建 Agent 前完成配置内 MCP 的发现，保留官方工具搜索和按需调用；已安装技能支持 `/技能名 任务描述` 及连续技能调用。
- 修复多图消息只传入第一张图片；新增 `/skills`、`/memory` 和技能面板“在聊天中使用”，帮助命令不再生成待回答的澄清卡片。
- 移除过时的 Windows 等待进程补丁，修复新版 Hermes 的 `bounded_capture` 参数引发文件工具失败，并恢复官方输出限制和中断处理。
- 工具失败按官方结果检测，任务取消后停止工具等待状态；较短的最终答案不再被较长的中间过程覆盖。

### 性能优化：缓存与上下文

- 恢复压缩会话时只载入当前摘要与有效尾部，避免重放旧历史；保留稳定的系统提示、工具定义及协议缓存机制。
- 模型窗口按服务地址和模型精确解析并传给压缩器，修复模型切换和配置修改后窗口沿用旧值。
- 分开显示当前窗口占用与工具循环累计用量；缓存读取、写入和按 Token 加权的命中率使用服务商实际报告，压缩后未测量时标为估算。

### 验证与边界

- 自动回归、真实固定版本 Hermes 集成和安装包检查见 [v0.2.33 验收记录](VALIDATION_0.2.33.md)。
- `/compact`、`/goal` 等未接入的交互命令会保留草稿并明确提示；未宣称完整复刻官方终端界面。
- 实际云端缓存命中率取决于模型、服务商与请求内容，本地协议测试不代表云端性能提升比例。

## Hermes Forge v0.2.32

发布日期：2026-09-17

### 核心修复

- 修复首次打开时 IPC 尚未注册造成的偶发失败；macOS 重建窗口后仍能正确调用客户端接口。
- 本地缓存损坏、存储受限或写满时自动降级，界面异常提供恢复入口；仅持久化必要偏好，历史任务从本地记录恢复。
- 安装阶段提前准备 OpenAI / Anthropic 两种聊天协议需要的依赖，修复先装 Hermes、再配置 Kimi / MiniMax / Anthropic 时缺少 SDK 的问题。
- 统一寻找已安装的 uv、Git、Git Bash 和受管 Python，减少因桌面进程 PATH 不完整造成的误报；取消安装后不再继续校验或误报成功。
- 更新 Electron 41.10.7 和自动更新组件的兼容修复版本，生产 npm 依赖审计为 0 项。

### 体验优化

- 首次引导等待模型和关键依赖检查完成后再继续，检查超时可重试；未完成的检查不会显示成“模型已就绪”。
- 源码与依赖下载持续显示日志和等待提示；失败或取消后保留日志入口，避免重复点击启动多次安装。
- 模型保存后提供“开始第一条对话”；保存成功但列表刷新失败时保留正确结果并提示刷新。
- Windows 复用已有工具，仅下载缺失工具的引导脚本；macOS 自动准备固定版本 uv，校验安装器后写入独立工具目录。
- 安装向导说明首次联网和模型配置步骤；GitHub Release 自动附带本版本说明。
- 未主动检测的环境显示“待检查”，不再持续转圈；macOS 选中其他项目的 Git 仓库时会停止安装，避免改动已有项目。

### 验证

- 验证结果与平台限制详见 [v0.2.32 验收记录](VALIDATION_0.2.32.md)。

### 已知限制

- 首次安装 Hermes 仍需联网；社区选项仅替换 Windows 工具引导脚本，源码仍从官方 GitHub 下载。
- macOS 缺少 Git 时需先安装系统命令行工具；本地开发环境为 Windows，macOS 打包与离线启动由 CI 验证。

## Hermes Forge v0.2.31

发布日期：2026-09-15

### 核心修复

- 对齐官方 Hermes 0.21.3 / v2026.9.14，固定提交 `345cd2b057a452236de401d3534b8502a7465e8d`；统一安装、更新和兼容检查。
- 原地升级旧分支、标签和 detached HEAD；维护锁保护运行中任务，官方锁文件同步受管环境，支持中断后重试，不创建升级备份或自动 stash。
- 修复 Windows Store Python 环境重建失败，先验证 uv 管理的解释器再修复同一个环境；所有运行路径优先使用同一 `venv`。
- 接入官方工具审批与澄清回调，按请求 ID 双向传递；修复取消、异常退出和失败被误报为完成的问题。
- 模型与连接器配置共用按 Home 串行队列和原子写入，保留官方未知字段；同步失败时停止启动 Gateway。
- 会话查询使用只读数据库并关闭连接；清空旋转官方会话映射；导出真实消息；按精确所有权清理日志与快照。
- 修复飞书 Profile 文件复制后不更新、多实例健康状态和异常退出重启。
- 修复模型代理将上游 401 误报为 502 的问题，正确处理 JSON 请求、同源重定向与压缩响应；保留模型服务返回的真实错误。

### 体验调整

- 主导航收敛为聊天、工作区、技能与记忆、设置；项目与空间合并，Profile 移入高级设置。
- 移除独立 Kanban 及专属 IPC、无效命令和不完整的会话导入/复制入口。
- 连接器、定时任务和桌面自动化按需启用；保留已明确启用的迁移配置。WSL 仅用于迁移。

### 性能优化

- 设置、技能、记忆和扩展按需加载，分离 Markdown 依赖；首页预载 JavaScript 约从 635 KB 降至 252 KB。
- 被动 Gateway 状态读取使用进程、状态文件和短时缓存；仅显式刷新运行 CLI。
- 收窄 Renderer 状态订阅，打包仅保留主进程依赖、必要 JS/WASM 与中英文资源。
- 新增独立离线 `--smoke-test`，验证真实窗口、preload、IPC 与 SQL WASM；真实模型审计失败返回非零退出码。

### 验证

- 类型检查、生产构建通过；70 个 TypeScript 测试文件共 517 项测试、14 项 Python 测试全部通过。
- Windows 最终安装包为 89.48 MiB；五分钟默认空闲实测未启动外部进程，未产生 HTTP/WebSocket 请求。
- 本机 Hermes 原地升级至官方 0.21.3，重复升级通过；主 Python 环境约从 1,203 MiB 降至 216.8 MiB，依赖锁定为 101 个包。

### 验收限制

- 最终包完整模型审计曾因附件读取超时失败；同一最终包仅做一次附件单项复测，97.083 秒通过。该记录不计为整轮审计通过。
- 本机 MiMo 配置的现有密钥返回 HTTP 401，成功切换仍需更新有效密钥后验证。
- macOS 尚未实机验收；外部消息平台未进行真实收发联调。

完整结果、统计口径及本机清理例外详见 [本轮验收记录](VALIDATION_0.2.31.md)。

## Hermes Forge v0.2.30

发布日期：2026-08-03

这批变更集中优化从安装到日常使用的完整路径，同时收紧桌面端默认安全边界。

### 模型接入与 Hermes 兼容

- **对齐 Hermes Agent v0.19.1**：默认安装源锁定到已审计的 `v2026.7.30` 稳定 Tag，并直接使用官方安装器的 `-Tag` 能力；高级设置仍可选择其他分支或 commit。
- **聊天模型改为会话级选择**：输入框模型菜单和 `/model` 只影响当前会话，不再悄悄修改全局默认模型或重启 Gateway；全局默认仍在模型设置中显式维护。
- **连接结论不再过度乐观**：工具调用探针失败的模型只会标记为辅助模型；Coding Plan / Token Plan 只确认凭据就绪，不再误报“已保存、已同步、已完成运行验证”。
- **新增深度运行验证入口**：已保存模型可直接执行 Hermes 运行环境、Provider、密钥映射和运行时解析检查。
- **新增 OpenAI 官方 API 一等入口**：提供独立来源、默认地址、密钥引用和模型预设，不再要求用户通过“OpenAI-compatible”绕行配置。
- **上下文窗口改用真实探测值**：移除固定 `256k` 提示写入，优先采用 Provider 返回的上下文能力，避免上下文进度和主模型资格被虚高数据误导。
- **兼容 Hermes v33 配置**：同步模型时保留 `reasoning_effort`、`reasoning_overrides`、`max_tokens`、`api_mode`、辅助模型与 fallback 配置，并同步 `supports_vision`。
- **模型配置写入更可靠**：`config.yaml` 与 `.env` 改用临时文件原子替换；不再把运行时模型别名持久化回 Hermes `.env`。
- **密钥与地址处理更安全**：Gemini API Key 改走请求头，避免出现在 URL 和错误信息；Anthropic 地址不再套用 OpenAI `/v1` 规则；保存或删除密钥后立即失效运行时缓存。
- **OAuth / 本机凭据提示更准确**：Provider 元数据会按鉴权能力展示，不再把 OAuth、本机凭据和外部登录流程统一误标为“必须填写 API Key”。

### 使用体验

- 首次启动改为“环境检测 → 安装 Hermes → 配置模型”显式三步，不再自动弹出安装源，也不会在检测成功后自动跳页。
- 首启页改为双栏任务布局，依赖检查、自动修复、安装日志和下一步操作保持在同一视野内。
- 工作台空状态按“连接项目、描述目标、查看执行”组织，并提供可直接填入输入框的常见任务。
- 模型、Hermes、诊断或工作区阻塞会在输入框上方显示直接修复入口；状态栏异常可直达健康检查。
- 新增 `Ctrl/Cmd + K/N/O/B` 高频快捷键，并改善会话搜索、空状态、加载骨架、通知停留时间和键盘焦点。
- 设置中心统一中文导航并适配较窄窗口，保存反馈和当前分区始终可见。

### 安全与可靠性

- 默认 CLI 权限由 `yolo` 收紧为 `guarded`，Gateway 默认不随应用启动。
- 主窗口导航、重定向和所有 IPC 调用增加可信来源校验，渲染端增加 CSP。
- 密钥库写入串行化并使用临时文件原子替换，避免并发保存丢失或文件损坏。
- CI 与发布覆盖 Windows/macOS，发布前执行检查、测试、构建及安装包系统审计。

### 验证

- `npm run check` 通过
- `npm test` 通过，54 个测试文件、426 个测试全部成功
- `npm run build` 通过
- `git diff --check` 通过
