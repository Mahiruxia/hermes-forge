...
1. **降低 Hermes Agent 在 Windows 上的首次使用门槛。**
   应用内置首启体检、Hermes 自动安装、Git/Python/微信依赖检查、模型配置检测和健康状态展示；新安装和运行链路固定走 Windows Native，WSL 仅作为旧配置迁移来源。
...
- v0.2.3 进一步强化了一键诊断能力，覆盖 Windows Native 基础依赖自动检测与修复。
...
- **一键诊断**：覆盖当前运行环境、Hermes CLI、模型连接和系统能力的完整检测链；Windows Native 会检查本机 Git/Python、Hermes 路径和模型连通性；支持自动修复缺失依赖，并智能分类 pip 失败原因（权限、PEP 668、网络、ensurepip）。
