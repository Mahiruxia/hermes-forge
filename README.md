...
Hermes Forge 是以聊天为中心的 Hermes Agent 桌面助手，保留 Electron 架构。Windows 原生是主要验收平台；macOS 使用原生安装和运行策略，实际平台验收状态见 [验收记录](VALIDATION_0.2.33.md)。
...
桌面聊天直接使用官方 `AIAgent`，继承当前 Hermes Profile 的人格、推理偏好、工具集、回合与时间预算、备用模型、检查点、预置消息和服务商路由配置。MCP 在创建 Agent 前按启用列表发现，后续保留官方的工具搜索与按需调用机制。Windows 文件和终端工具使用官方的输出限制及中断处理。

输入 `/技能名 任务描述` 可调用已安装技能，也支持连续指定多个技能。`/skills`、`/memory` 打开对应面板；技能面板可把使用说明填入聊天草稿。`/help` 显示桌面支持的命令；`/compact`、`/goal` 暂未接入，自动上下文压缩仍由 Hermes 执行。

一条消息中的多张图片会一起交给 Hermes，并遵循当前模型的视觉能力及辅助视觉配置。工具失败会保留失败状态，Agent 可继续恢复并完成任务；最终答案以官方最终回复为准，取消后未完成的工具会结束等待状态。

## 模型缓存与上下文

聊天复用 Hermes 官方的模型缓存机制。恢复会话时只加载当前摘要与有效尾部消息，保留工具调用、推理内容和原始消息格式，避免把已经压缩的旧历史重新加入请求。模型窗口按对应服务地址的元数据或手动配置解析，并传给 Hermes 的压缩器。

输入框的上下文指示器显示最近一次请求、回复和当前草稿的占用；工具循环的累计 Token 用量单独展示。缓存命中率按上报的读取 Token 占完整输入的比例计算，侧栏按 Token 数加权汇总，缓存命中的内容仍占用上下文窗口。压缩后尚无实测值或切换模型时显示估算状态。

实际缓存命中还取决于服务商、模型、相同请求前缀及缓存有效期；本地验证范围见 [缓存与上下文验收记录](CACHE_CONTEXT_VALIDATION_2026-09-17.md)。
